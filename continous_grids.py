...
class GridWorld(gym.Env):
    ...
    def test_vis_trajectory(self, traj, heatmap_normalize=False, heatmap_vertical_clip_value=2500):
        ...
        for num in range(2, 6):
            ...
            x_bins = np.arange(self.min_position * self.scale, self.max_position * self.scale, num)
            y_bins = np.arange(self.min_position * self.scale, self.max_position * self.scale, num)
            h = ax.hist2d(x, y, bins=[x_bins, y_bins], cmap='Blues', normed=heatmap_normalize,
                          vmax=heatmap_vertical_clip_value)
            image = h[3]
            ...
            start_circle = patches.Circle(start_point, radius, facecolor='gold', edgecolor='black', lw=0.5, zorder=10)
            goal_circle = patches.Circle(goal_point, radius, facecolor='brown', edgecolor='black', lw=0.5, zorder=10)
            ...
            if self.num_rooms == 1:
                ...
                wall3_height = (self.max_position / 2 + self.wall_breadth - self.max_position / 4 - self.door_breadth
                                ) * self.scale
            ...
            # plt.savefig('./{}_{}_{}.pdf'.format(name_plot, experiment_id, num))
